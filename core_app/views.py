...
def index(request):
    ...
    user_following = FolowersCount.objects.filter(follower=request.user.username)
...
def profile(request, pk):
    ...
    if FolowersCount.objects.filter(follower=follower, user=user).first():
       button_text = 'Unfollow'
    else: 
      button_text = 'Follow'

    user_followers = len(FolowersCount.objects.filter(user=pk))
    user_following = len(FolowersCount.objects.filter(follower=pk))
...
def follow(request):
   if request.method == 'POST':
      ...
      if FolowersCount.objects.filter(follower=follower, user= user).first():
         delete_follower = FolowersCount.objects.get(follower=follower, user= user)
         delete_follower.delete()
         return redirect('/profile/'+user)
      else:
         new_follower = FolowersCount.objects.create(follower=follower, user=user)
         return redirect('/profile/' + user)
   else:
      return redirect('/')
